...
class GoalsRepositoryImpl(
    ...
    override suspend fun logReactionAdded(userId: String) {
    ...
    // called from homeViewModel's setReaction
    // called only if the reaction was deleted, to remove them from missions that are incomplete
    override suspend fun logReactionRemoved(userId: String) {

        // update missions of type "react_to_article"
        val missions = getMissions(userId)
        missions.filter { it.type == "react_to_article" && !it.isCompleted }.forEach { mission ->
            val newCount = (mission.currentCount - 1).coerceAtLeast(0)
            updateMissionProgress(userId, mission.id, newCount)
        }
    }
